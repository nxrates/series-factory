...
use nxr_sdk::asset_class::{
    bucket_for_pair, effective_list, DEFAULT_CRYPTO_MAJORS, DEFAULT_FX_MAJORS, DEFAULT_STABLECOINS,
};
...
use series_factory::bar_construction::{
    build_vol_from_s10, count_bars_per_day_from_prices, scale_to_target_k, CalibrationConfig,
    S10ShardIter,
};
...
fn main() -> Result<()> {
    ...
    for pair in &pairs {
        let m = match resolve_ticker(pair, InstrumentType::SPOT) {
            Ok(m) => m,
            Err(e) => {
                warn!(pair, ?e, "resolve failed; skipping");
                continue;
            }
        };
        ...
        let prices = match load_full_tick(&idx_root, ticker_id) {
            ...
            Ok(_) => {
                warn!(pair, "empty idx; skipping");
                continue;
            }
            Err(e) => {
                warn!(pair, ?e, "idx load failed; skipping");
                continue;
            }
        };
        ...
        for &w in &windows {
            ...
            for i in 1..=args.anchors {
                ...
                let err = if stats.days == 0 {
                    f64::NAN
                } else {
                    (stats.median / target - 1.0).abs()
                };
                all.push((
                    pair.clone(),
                    Fold {
    ...
    for &w in &windows {
        let folds: Vec<&Fold> = all
            .iter()
            .filter(|(_, f)| f.kind == "fwd" && f.window_days == w)
            .map(|(_, f)| f)
            .collect();
        let ok: Vec<f64> = folds
            .iter()
            .filter(|f| f.err.is_finite())
            .map(|f| f.err)
            .collect();
